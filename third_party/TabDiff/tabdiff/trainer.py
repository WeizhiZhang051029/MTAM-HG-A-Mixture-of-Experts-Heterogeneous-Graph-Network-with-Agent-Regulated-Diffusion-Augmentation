...
class Trainer:
    def __init__(
        ...
        self.steps = steps
        self.init_lr = lr
        trainable_params = [param for param in self.diffusion.parameters() if param.requires_grad]
        if not trainable_params:
            raise ValueError("No trainable parameters are enabled for TabDiff training.")
        ...
        self.min_save_epoch = int(min_save_epoch)
        self.reset_train_epoch = bool(reset_train_epoch)
    ...
    def run_loop(self):
        ...
        for epoch in range (start_epoch, self.steps):
            ...
            if total_loss < best_loss and self.curr_epoch >= self.min_save_epoch:
            ...
            if ema_total_loss < best_ema_loss and self.curr_epoch >= self.min_save_epoch:
            ...
            if (epoch+1) % self.check_val_every == 0:
                ...
                ema_out_metrics, _, _ = self.evaluate_generation(ema=True, save_metric_details=True)
                log_dict.update({
                    "ema": ema_out_metrics,
                })
    ...
    def evaluate_generation(self, save_metric_details=False, ema=False):
        self.diffusion.eval()
