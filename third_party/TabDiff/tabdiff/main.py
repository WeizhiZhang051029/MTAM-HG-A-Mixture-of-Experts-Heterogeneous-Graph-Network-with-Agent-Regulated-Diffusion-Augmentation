...
from tabdiff.models.capl_mechanism import CAPLMechanismConstraint, CAPLMechanismWeights
...
def apply_cli_overrides(raw_config, args):
    if getattr(args, "finetune_lr", None) is not None:
        raw_config["train"]["main"]["lr"] = float(args.finetune_lr)
    if getattr(args, "finetune_steps", None) is not None:
        raw_config["train"]["main"]["steps"] = int(args.finetune_steps)
    if getattr(args, "num_timesteps_override", None) is not None:
        raw_config["diffusion_params"]["num_timesteps"] = int(args.num_timesteps_override)
    if getattr(args, "stochastic_sampler", None) is not None:
        raw_config["diffusion_params"]["sampler_params"]["stochastic_sampler"] = str(args.stochastic_sampler).lower() == "true"
    return raw_config


def apply_trainable_scope(model, scope):
    scope = str(scope or "all").lower()
    if scope == "all":
        return
    for param in model.parameters():
        param.requires_grad = False
    if scope == "mlp_detokenizer":
        modules = [
            model.denoise_fn_D.denoise_fn_F.mlp,
            model.denoise_fn_D.denoise_fn_F.detokenizer,
        ] if getattr(model, "precond", False) else [model.denoise_fn_D.mlp, model.denoise_fn_D.detokenizer]
    elif scope == "mlp":
        modules = [model.denoise_fn_D.denoise_fn_F.mlp] if getattr(model, "precond", False) else [model.denoise_fn_D.mlp]
    elif scope == "detokenizer":
        modules = [model.denoise_fn_D.denoise_fn_F.detokenizer] if getattr(model, "precond", False) else [model.denoise_fn_D.detokenizer]
    else:
...
def build_capl_mechanism(args, data_dir, info, train_data):
    if not args.mechanism_constraint:
        return None
    weights = CAPLMechanismWeights(
        temperature_path=args.mechanism_temperature_weight,
        production_window=args.mechanism_window_weight,
        yield_residual=args.mechanism_yield_weight,
    )
    return CAPLMechanismConstraint.from_tabdiff_dataset(
        data_dir,
        info,
        ...
        ridge_alpha=args.mechanism_ridge_alpha,
        temperature_hold_tolerance=args.mechanism_temperature_hold_tolerance,
        yield_tolerance=args.mechanism_yield_tolerance,
        weights=weights,
        enabled=True,
    )
...
def main(args):
    ...
    if args.mode == 'train':
        ...
        raw_config = apply_cli_overrides(cached_raw_config, args)
    ...
    mechanism_constraint = build_capl_mechanism(args, data_dir, info, train_data)
    if mechanism_constraint is not None:
        mechanism_constraint.to(device)
        raw_config["mechanism_constraint"] = {
            "enabled": True,
            "mechanism_lambda": args.mechanism_lambda,
            "guidance_scale": args.guidance_scale,
            "trainable_scope": args.trainable_scope,
            "temperature_hold_tolerance": args.mechanism_temperature_hold_tolerance,
            "yield_tolerance": args.mechanism_yield_tolerance,
            "temperature_weight": args.mechanism_temperature_weight,
            "window_weight": args.mechanism_window_weight,
            "yield_weight": args.mechanism_yield_weight,
        }
    ...
    diffusion = UnifiedCtimeDiffusion(
        ...
        mechanism_lambda=args.mechanism_lambda if mechanism_constraint is not None else 0.0,
        guidance_scale=args.guidance_scale if mechanism_constraint is not None else 0.0,
