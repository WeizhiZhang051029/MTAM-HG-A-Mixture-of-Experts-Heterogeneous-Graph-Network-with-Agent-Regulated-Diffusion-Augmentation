...
class UnifiedCtimeDiffusion(torch.nn.Module):
    def __init__(
        ...
        self.mechanism_constraint = mechanism_constraint
        self.mechanism_lambda = float(mechanism_lambda)
        self.guidance_scale = float(guidance_scale)
        self.guidance_start_step = int(guidance_start_step)
        self.guidance_stop_step = guidance_stop_step
    ...
    def mechanism_loss(self, x_num_pred):
        if self.mechanism_constraint is None or self.mechanism_lambda <= 0:
            return x_num_pred.new_tensor(0.0)
        if x_num_pred.shape[1] != self.num_numerical_features:
            return x_num_pred.new_tensor(0.0)
        return self.mechanism_constraint(x_num_pred, reduce=True)
    ...
    def mixed_loss(self, x):
        ...
        if x_num.shape[1] > 0:
            ...
            mech = self.mechanism_loss(model_out_num)
            if self.mechanism_lambda > 0:
                c_loss = c_loss + self.mechanism_lambda * mech
        if x_cat.shape[1] > 0:
    ...
    def sample(self, num_samples):
        ...
        with torch.no_grad():
            for i in pbar:
                ...
                if self._guidance_enabled(i):
                    z_norm = self.apply_mechanism_guidance(z_norm, i)
        
        assert torch.all(z_cat < self.mask_index)
        sample = torch.cat([z_norm, z_cat], dim=1).cpu()
        return sample

    def _guidance_enabled(self, i):
        if self.mechanism_constraint is None or self.guidance_scale <= 0:
            return False
        if i < self.guidance_start_step:
            return False
        if self.guidance_stop_step is not None and i > int(self.guidance_stop_step):
            return False
        return True

    def apply_mechanism_guidance(self, x_num, i):
        if x_num.shape[1] != self.num_numerical_features:
            return x_num
        with torch.enable_grad():
            ...
            if not torch.isfinite(grad).all():
                return x_num
            scale = self.guidance_scale * (float(i) + 1.0) / max(float(self.num_timesteps), 1.0)
            updated = guided - scale * grad
